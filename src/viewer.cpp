        ...
        if (e.path().extension() == ".ply") {
          ...
          if (ImGui::Selectable(fn.c_str())) {
            ...
            if (M.isValid()) {
              ...
              originalVertices.resize(M.vertices.size(), 3);
              for (size_t i = 0; i < M.vertices.size(); i++) {
                originalVertices.row(i) = M.vertices[i];
              }
            }
  ...
  if (deformationModeEnabled) {
    ...
    if (isDragging && ImGui::IsMouseDown(0) && draggedVertexIndex >= 0) {
      ...
      if (dt >= sampleInterval && inside) {
        ...
        // Update the vertex position in our copy and in the mesh
        if (draggedVertexIndex < originalVertices.rows()) {
          originalVertices.row(draggedVertexIndex) = wp;
          currentMesh->updateVertexPositions(originalVertices);
        }
